	...
	while (!glfwWindowShouldClose(mainWindow)) {
		...
		if (settings.optimization) {
                ...
                if (currentTime - startTime >= settings.optiDuration) {
                        ...
                        float totalError = m_optiData.currentCost;
                        std::cout<<"Cout " << m_optiData.currentCost << std::endl;
                        /// ========================================== ///
                        ...
                        for (int i=0; i<m_optiData.nbParam; i++) {
                                ...
                                float signe = RandomFloat(0.0,1.0)-0.5f;
                                if(signe<0.0){signe=-1.0f;}
                                else{signe=1.0f;}
                                // Valeur max de l'incrément (diminue avec les itérations)
                                float max_val =std::exp(-m_optiData.nbIter*0.001f)*m_optiData.bestParam[i]*0.2f;
                                // Incrément aléatoire (vous pouvez utiliser RandomFloat(a,b))
                                float increment = RandomFloat(0.0,max_val);
                                // Application de l'incrément
                                m_optiData.currentParam[i]=m_optiData.bestParam[i]+signe*increment;
                                /// ========================================== ///
                        }
